...
> 「==QuasiConnectivity（半连接性）=={.important}这个特性是与活塞一同引入游戏的，因为它直接复制了门的代码，这是一个bug，但后来因玩家要求被官方认可并保留了下来。」

不知道从什么时候开始，大多数Minecraft玩家心中都埋下了这样的印象。

![都这么想](media/IMG_0713.jpeg)
...
* 门：Infdev 2010 加入
* 发射器：Beta 1.2加入
* ==活塞：Beta 1.7加入=={.important}
* 投掷器：正式版 1.5加入

令人意外的是，发射器在Beta 1.2中就==已经具备QC特性=={.important}。这比活塞要早了几乎半年。
...
> world.isBlockIndirectlyProvidingPowerTo(x, y, z, 0) || // [!code focus]
> world.isBlockIndirectlyProvidingPowerTo(x, y + 2, z, 1) || // [!code focus]
> world.isBlockIndirectlyProvidingPowerTo(x, y + 1, z - 1, 2) || // [!code focus]
> world.isBlockIndirectlyProvidingPowerTo(x, y + 1, z + 1, 3) || // [!code focus]
> world.isBlockIndirectlyProvidingPowerTo(x - 1, y + 1, z, 4) || // [!code focus]
> world.isBlockIndirectlyProvidingPowerTo(x + 1, y + 1, z, 5); // [!code focus]
> }
> ```
...
* **2011年3月**，==第三方mod作者Hippoplatimus发布了**Pistons Mod**=={.important}，首次实现了活塞，该mod被jeb看好，==获得并整合了该mod的代码进入Minecraft=={.important}。
* **2011年6月**，Jeb在Twitter上公布了游戏内活塞的早期版本截图。
* **2011年7月**，活塞随Beta 1.7加入Minecraft，具有QC。
* **2013年3月**，正式版1.5推出，投掷器加入，具有QC。

![Pistons](media/IMG_0712.jpeg)

奇怪的是，Pistons Mod中的代码中，也出现了能够导致QC的额外充能检测逻辑。但这段逻辑，与Beta 1.7版本中官方活塞所用的QC实现方式又==完全不同。=={.important}
...
>  var1.q(var2, var3 + 1, var4); // [!code focus]
> ```
...
似乎难以“巧合”或“失误”来解释。这样一看，：==QC似乎一开始就是一个“设计”，而不是一个“bug”=={.important}。
