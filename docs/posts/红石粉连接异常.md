...
最后，查询 `ShulkerBoxBlock.java` 中对 `getShape` 的覆盖，我们得以验证这个问题：
...
可以看到，潜影盒的碰撞体形状**依赖于从 `pos` 获取方块实体**。  
当第一段代码错误地传入了红石粉自己的 `pos` 时，`level.getBlockEntity(pos)` 拿到的是红石粉位置的方块实体（不存在），于是直接返回 `Shapes.block()`（完整方块）。这便正是开启的潜影盒被误判为实心的直接原因。

---

由此可见，问题的本质是：

> `isRedstoneConductor` 依赖方块的碰撞体形状判断是否实心，而潜影盒的 `getShape` 又依赖 `pos` 获取方块实体。红石粉中，判断是否可以向上连接的代码错误地传入了红石粉自己的 `pos`，导致获取不到潜影盒的方块实体，于是返回完整方块形状，使**开启的潜影盒被误判为实心**。
...
由于默认的 `getShape` 实现会忽略 `pos` 参数，这一问题在绝大多数方块中不会暴露，属于**隐性问题**。在原版 Minecraft 中，只有潜影盒等少数依赖方块实体动态形状的方块会触发此问题。

不过，这一隐患在模组环境中可能被放大，或许值得模组开发者们留意。
